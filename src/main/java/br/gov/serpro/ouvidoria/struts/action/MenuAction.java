...
public class MenuAction extends ActionSupport {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String imageLogo = config.getImagemLogo();
		if (imageLogo.indexOf("/") == 0) {
			imageLogo = imageLogo.substring(1);
		}
		request.setAttribute("imageLogo", imageLogo);

		request.setAttribute("nomeExibicaoOrgao", config
						.getNomeExibicaoOrgao());

		String imageBanner = config.getImagemBanner();
		if (imageBanner.indexOf("/") == 0) {
			imageBanner = imageBanner.substring(1);
		}
		request.setAttribute("imageBanner", imageBanner);
